...
namespace canvas
{
    ...
    constexpr auto rgb(uint8_t red, uint8_t green, uint8_t blue) -> uint32_t
    {
        return blue | (green << 8) | (red << 16) | 0xff000000;
    }

    constexpr auto rgba(uint8_t red, uint8_t green, uint8_t blue, uint8_t alpha) -> uint32_t
    {
    ...
    constexpr auto rgba(int red, int green, int blue, int alpha) -> uint32_t
    {
        return rgba(
            static_cast<uint8_t>(std::max(0, std::min(255, red))),
            static_cast<uint8_t>(std::max(0, std::min(255, green))),
    ...
    constexpr auto unpack_color(uint32_t color) -> std::tuple<int, int, int, int>
    {
        return {
            red_of(color),
            green_of(color),
    ...
    constexpr auto blend_color(uint32_t begin, uint32_t end, uint8_t factor) -> uint32_t
    {
        auto f0 = 0xff - factor;

        auto r = std::max(0, std::min((red_of(begin) * f0 + red_of(end) * factor) / 0xff, 0xff));
        auto g = std::max(0, std::min((green_of(begin) * f0 + green_of(end) * factor) / 0xff, 0xff));
        auto b = std::max(0, std::min((blue_of(begin) * f0 + blue_of(end) * factor) / 0xff, 0xff));
