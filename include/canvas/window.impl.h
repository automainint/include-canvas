...
namespace canvas
{
    static constexpr uint32_t _window_style = WS_CAPTION | WS_SYSMENU | WS_MINIMIZEBOX | WS_VISIBLE;
    ...
    inline auto CALLBACK _window_proc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) -> LRESULT
    {
        LRESULT result = 0;
    ...
    inline auto _adjust_rect() -> std::tuple<int, int, int, int>
    {
        ...
        r.left      = 0;
        r.top       = 0;
        r.right     = static_cast<LONG>(_ref().frame_width);
        r.bottom    = static_cast<LONG>(_ref().frame_height);
    ...
    inline void _init_buffer()
    {
        ...
        _ref().info.bmiHeader.biSizeImage   = static_cast<DWORD>(_ref().frame_width * _ref().frame_height * sizeof(uint32_t));
    }
