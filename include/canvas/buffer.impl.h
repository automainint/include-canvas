...
namespace canvas
{
    ...
    {
        return _check(x, static_cast<int>(m_width)) && _check(y, static_cast<int>(m_height));
    }
    ...
    inline auto buffer::_clamp_relative(int x0, int y0, int x1, int y1) const -> std::tuple<int, int, int, int>
    {
        int i0 = x0, i1 = x1;
        int j0 = y0, j1 = y1;
    ...
    inline void buffer::set_size(size_t width, size_t height, uint32_t default_color)
    {
        m_width = width;
        m_height = height;
    ...
    inline void buffer::draw_horisontal(int x0, int x1, int y, uint32_t color)
    {
        _clamp(x0, x1, m_width);
        _put_line(x0, x1, y, color);
    }

    inline void buffer::draw_vertical(int x, int y0, int y1, uint32_t color)
    {
    ...
    inline void buffer::draw_rectangle(int x0, int y0, int x1, int y1, uint32_t color)
    {
    ...
    inline void buffer::fill_rectangle(int x0, int y0, int x1, int y1, uint32_t color)
    {
        _clamp(x0, x1, m_width);
        _clamp(y0, y1, m_height);
    ...
    inline void buffer::draw_line(int x0, int y0, int x1, int y1, uint32_t color)
    {
        ...
        if (dx > dy)
        {
            ...
            for (int x = 0; x <= dx; x++)
            {
                ...
                put_pixel(x0 + x, y0 + y + 1,   rgba(r, g, b, static_cast<int>(a * k0 + .5)));
                put_pixel(x0 + x, y0 + y,       rgba(r, g, b, static_cast<int>(a * k1 + .5)));
            }
        }
        else if (dy > 0)
        {
            ...
            for (int y = 0; y <= dy; y++)
            {
                ...
                put_pixel(x0 + x + 1,   y0 + y, rgba(r, g, b, static_cast<int>(a * k0 + .5)));
                put_pixel(x0 + x,       y0 + y, rgba(r, g, b, static_cast<int>(a * k1 + .5)));
            }
        }
        else
    ...
    inline void buffer::draw_line_fast(int x0, int y0, int x1, int y1, uint32_t color)
    {
    ...
    inline void buffer::put_pixel(double x, double y, uint32_t color)
    {
        ...
        put_pixel(ix0,      iy0 + 1,    rgba(r, g, b, static_cast<int>(k01 * a + .5)));
        put_pixel(ix0 + 1,  iy0,        rgba(r, g, b, static_cast<int>(k10 * a + .5)));
        put_pixel(ix0 + 1,  iy0 + 1,    rgba(r, g, b, static_cast<int>(k11 * a + .5)));
    }

    inline void buffer::draw_line(double x0, double y0, double x1, double y1, uint32_t color)
    {
        const auto [r, g, b, a] = unpack_color(color);
        ...
        if (dx > dy)
        {
            ...
            for (int x = 1; x < idx; x++)
            {
                const auto fy = oy0 + ((static_cast<double>(x) - ox0) * sdy) / dx;
                const auto fy0 = std::floor(fy);
                ...
                put_pixel(ix0 + x, iy0 + y + 1, rgba(r, g, b, static_cast<int>(a * k0 + .5)));
                put_pixel(ix0 + x, iy0 + y,     rgba(r, g, b, static_cast<int>(a * k1 + .5)));
            }

            {
                ...
                put_pixel(ix0, iy0 + y + 1, rgba(r, g, b, static_cast<int>(a * k0 + .5)));
                put_pixel(ix0, iy0 + y,     rgba(r, g, b, static_cast<int>(a * k1 + .5)));
            }

            {
                const auto fy = oy0 + ((static_cast<double>(idx) - ox0) * sdy) / dx;
                const auto fy0 = std::floor(fy);
                ...
                put_pixel(ix1, iy0 + y + 1, rgba(r, g, b, static_cast<int>(a * k0 + .5)));
                put_pixel(ix1, iy0 + y,     rgba(r, g, b, static_cast<int>(a * k1 + .5)));
            }
        }
        else if (dy > 0)
        {
            ...
            for (int y = 1; y < idy; y++)
            {
                const auto fx = ox0 + ((static_cast<double>(y) - oy0) * sdx) / dy;
                const auto fx0 = std::floor(fx);
                ...
                put_pixel(ix0 + x + 1,  iy0 + y, rgba(r, g, b, static_cast<int>(a * k0 + .5)));
                put_pixel(ix0 + x,      iy0 + y, rgba(r, g, b, static_cast<int>(a * k1 + .5)));
            }

            {
                ...
                put_pixel(ix0 + x + 1,  iy0, rgba(r, g, b, static_cast<int>(a * k0 + .5)));
                put_pixel(ix0 + x,      iy0, rgba(r, g, b, static_cast<int>(a * k1 + .5)));
            }

            {
                const auto fx = ox0 + ((static_cast<double>(idy) - oy0) * sdx) / dy;
                const auto fx0 = std::floor(fx);
                ...
                put_pixel(ix0 + x + 1,  iy1, rgba(r, g, b, static_cast<int>(a * k0 + .5)));
                put_pixel(ix0 + x,      iy1, rgba(r, g, b, static_cast<int>(a * k1 + .5)));
            }
        }
        else
        {
    ...
    inline void buffer::draw_towards(double x0, double y0, double x1, double y1, uint32_t color)
    {
        double vx = x0 - x1;
        double vy = y0 - y1;
    ...
    inline void buffer::copy_sprite(int x0, int y0, size_t width, std::span<const uint32_t> pixels)
    {
        if (width > 0)
        {
            auto [i0, j0, i1, j1] =
            ...
            for (size_t n = n0; n <= n1; n += width, i += static_cast<int>(m_width))
            {
                std::copy(
                    pixels.begin() + n + i0,
                    pixels.begin() + n + i1 + 1,
    ...
    inline void buffer::copy_sprite_masked(int x0, int y0, size_t width, std::span<const uint32_t> pixels)
    {
        if (width > 0)
        {
            auto [i0, j0, i1, j1] =
    ...
    inline void buffer::draw_sprite(int x0, int y0, size_t width, std::span<const uint32_t> pixels)
    {
        if (width > 0)
        {
            auto [i0, j0, i1, j1] =
        ...
        for (size_t j = 0; j < buf.m_height; j++)
        {
            for (size_t i = 0; i < buf.m_width; i++)
            {
                ...
                for (size_t y = y0; y < y1; y++)
                {
                    ...
                    {
                        const auto i_dst = _index(static_cast<int>(x), static_cast<int>(y));
                        const auto i_src = buf._index(static_cast<int>(i), static_cast<int>(j));
